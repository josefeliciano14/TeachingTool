...
CREATE TABLE Modules(mid INT PRIMARY KEY  AUTO_INCREMENT NOT NULL, name varchar(255), description TEXT, creator int not null, FOREIGN KEY (creator) REFERENCES Users(uid), is_public boolean default false, code varchar(255) default null, hasImage boolean, date_created datetime default now());
...
CREATE TABLE Questions(qid INT PRIMARY KEY AUTO_INCREMENT NOT NULL, evaluation INT NOT NULL, FOREIGN KEY (evaluation) REFERENCES Evaluations(eid), prompt text NOT NULL, information TEXT, hasImage boolean, ind INT NOT NULL);
